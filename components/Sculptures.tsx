...
import type { Paintings, Sculptures } from "@/lib/types";
...
export default function Sculptures({
  ...
  const safeSlides = useMemo(() => sculptures ?? [], [sculptures]);
  ...
  const prev = () => {
    const nextIndex =
      (index - 1 + safeSlides.length) % safeSlides.length;
    goTo(nextIndex, "prev");
  };
  ...
  if (safeSlides.length === 0) {
    return (
      <div className="h-72 w-full rounded-2xl border bg-neutral-50 flex items-center justify-center text-sm text-neutral-500">
        Add sculpture images in /public/images and update mockContent.ts
      </div>
    );
  }
  ...
  const previousImage =
    prevIndex !== null ? safeSlides[prevIndex] : null;

  const currentText = safeSlides[displayIndex];

  const leavingTo =
    dir === "next" ? "-translate-x-[8%]" : "translate-x-[8%]";
  ...
  return (
    <div className="w-full">
      <div className="mx-auto space-y-6">
        ...
        <div className="relative mx-auto w-4/7 max-w-2xl aspect-square overflow-hidden rounded-3xl shadow">
          ...
          <div className="flex items-center justify-center gap-3 pt-2">
            {safeSlides.map((_, i) => (
              <button
                key={i}
                onClick={() =>
                  goTo(i, i > displayIndex ? "next" : "prev")
                }
